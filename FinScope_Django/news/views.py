...
def scrape_news_view(request):
    ...
    source_host = urlparse(news_url).hostname if urlparse(news_url).hostname else "Unknown source"
    ...
    try:
        ...
        if og_image_tag and og_image_tag.get('content'):
            scraped_image_url = og_image_tag['content']
        ...
        if source_host == 'www.newstomato.com':
            ...
            newstomato_selector = 'div.view_content' # Updated selector based on inspection
            # Prepend so it's tried first for this specific domain
            content_selectors.insert(0, newstomato_selector)
            print(f"Using specific selector for newstomato.com: {newstomato_selector}")
        elif source_host == 'www.financialpost.co.kr':
            financialpost_selector = '#article-view-content-div' # Selector from inspection
            content_selectors.insert(0, financialpost_selector)
            print(f"Using specific selector for www.financialpost.co.kr: {financialpost_selector}")
        ...
        for selector in content_selectors:
            try:
                ...
                if article_content_element:
                    break
            except Exception as e_select: # Catch potential errors from complex selectors
                print(f"Selector error for '{selector}': {e_select}")
                continue
        ...
        if article_content_element:
            ...
            irrelevant_selectors_in_content = ['.related-articles', '.comments-section', '.author-bio', '.share-buttons', '.ad-container']
            ...
            paragraphs = article_content_element.find_all('p', recursive=True) # Get all p tags, even nested
            if paragraphs:
                ...
                scraped_content = article_content_element.get_text(separator="\n", strip=True)
        else:
            ...
            if body_paragraphs:
                scraped_content = "\n\n".join(p.get_text(separator=" ", strip=True) for p in body_paragraphs if p.get_text(strip=True))

        # Final check if content is still placeholder or empty or too short
        if not scraped_content.strip() or \
           scraped_content.startswith("Placeholder content for") or \
           len(scraped_content.strip()) < 150: # Increased minimum length for meaningful content
        ...
        print(f"Error fetching URL {news_url}: {e}")
        # Keep default title and content if scraping fails
        scraped_content = f"Could not fetch or parse the news article from {news_url}. Error: {e}"
    except Exception as e:
        print(f"An unexpected error occurred during scraping {news_url}: {e}")
