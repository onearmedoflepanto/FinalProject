...
from accounts.models import FinancialProduct, ProductOption, SavedFinancialProduct # Import SavedFinancialProduct
    ...
    def handle(self, *args, **options):
        ...
        for product_type_key, endpoint in PRODUCT_TYPES.items():
            ...
            for base_info in base_list:
                ...
                # Retroactively link existing SavedFinancialProducts that might be missing the reference
                updated_saved_products_count = SavedFinancialProduct.objects.filter(
                    fin_co_no=fin_co_no,
                    fin_prdt_cd=fin_prdt_cd,
                    product_type=product_type_key,
                    financial_product_ref__isnull=True # Only update if ref is missing
                ).update(financial_product_ref=financial_product)

                if updated_saved_products_count > 0:
                    self.stdout.write(self.style.SUCCESS(
                        f"Linked {updated_saved_products_count} existing SavedFinancialProduct(s) to '{financial_product.fin_prdt_nm}'."
                    ))
