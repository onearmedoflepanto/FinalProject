...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    async bookmarkVideo(videoData) {
      if (!this.accessToken) {
        return Promise.reject(new Error('Not authenticated'));
      }
      try {
        const response = await api.post('api/accounts/bookmark-video/', videoData);
        // Optionally, update local user state if it includes bookmarked videos and needs to be fresh
        // For example, if this.user.bookmarked_videos exists:
        // if (this.user && this.user.bookmarked_videos) { // This direct push might lead to duplicates if not careful
        //   this.user.bookmarked_videos.push(response.data);
        // }
        await this.loadUser(); // Refresh user data to include the new bookmark
        return { success: true, data: response.data };
      } catch (error) {
        console.error('Failed to bookmark video:', error.response ? error.response.data : error.message);
        // Pass along the error message from the backend if available
        throw error; // Re-throw to be caught by the component
      }
    },
    async saveAiRecommendation(recommendationPayload) {
      if (!this.accessToken) {
        return Promise.reject(new Error('Not authenticated'));
      }
      try {
        const response = await api.post('api/accounts/save-ai-recommendation/', recommendationPayload);
        // Optionally, update local user state if it includes saved recommendations
        // This might require a re-fetch of user data or specific update logic
        // For now, we assume MyPage will fetch the latest saved recommendations when visited.
        return { success: true, data: response.data };
      } catch (error) {
        console.error('Failed to save AI recommendation:', error.response ? error.response.data : error.message);
        throw error; // Re-throw to be caught by the component
      }
    }
  }
