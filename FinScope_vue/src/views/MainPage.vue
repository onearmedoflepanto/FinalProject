          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-8 md:gap-10 items-stretch">
            <div class="info-box-container bg-gray-900 p-6 rounded-lg shadow-lg">
              ...
              <div class="info-box-content">
                ...
                <div class="mt-4 flex-grow bg-gray-700 rounded p-4 flex items-center justify-center text-gray-400 text-sm min-h-[150px]">
                  <exchangeRateChart />
                </div>
              </div>
            </div>
            ...
            <div class="info-box-container bg-gray-900 p-6 rounded-lg shadow-lg">
              ...
              <div class="info-box-content news-scroll max-h-60 overflow-y-auto">
...
const smoothScrollTo = (selector) => {
  ...
  if (element) {
    ...
    let offset = primaryNavHeight + navHeight + 20; 
    if (selector === '#introduction') { 
        offset = primaryNavHeight + 20; 
    }
...
const handleScroll = () => {
  ...
  let current = ''; 
  const navHeight = nav?.offsetHeight || 0;
  const detectionOffset = primaryNavHeight + navHeight + 60; 

  for (let i = sections.length - 1; i >= 0; i--) {
    const id = sections[i];
    const section = document.getElementById(id);
    if (section) {
      if (section.offsetTop <= scrollPosition + detectionOffset) {
        current = id;
        break; 
      }
    }
  }
  if (!current && scrollPosition < (document.getElementById(sections[0])?.offsetTop - detectionOffset || 0) ) {
    current = sections[0];
  }
  activeSection.value = current;
  // Note: The direct class manipulation for navLinks is removed from here.
  // Vue's reactive :class binding in the template will handle the style changes.
};
