<style scoped>
/* Ensure body and main take up full height if needed, though App.vue usually handles this */
body,
#app,
main {
  min-height: 100vh;
...
.form-input-custom {
  @apply w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition duration-150 ease-in-out;
  font-size: 0.95rem;
}

.form-select-custom {
  @apply w-full pl-4 pr-10 py-2.5 border border-gray-300 rounded-lg shadow-sm bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition duration-150 ease-in-out;
  font-size: 0.95rem;
  appearance: none;
...
.btn-primary {
  @apply w-full bg-teal-600 text-white font-semibold py-3 px-4 rounded-lg shadow-md hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 transition duration-150 ease-in-out;
}

.btn-secondary {
  @apply w-full bg-sky-500 text-white font-semibold py-2.5 px-4 rounded-lg shadow-md hover:bg-sky-600 focus:outline-none focus:ring-2 focus:ring-sky-400 focus:ring-offset-2 transition duration-150 ease-in-out;
  font-size: 0.9rem;
}

.radio-label-custom {
  @apply flex items-center text-gray-700 hover:text-teal-600 cursor-pointer;
}

.radio-custom {
  @apply appearance-none h-5 w-5 border-2 border-gray-400 rounded-full checked:bg-teal-500 checked:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-offset-1 transition duration-150 ease-in-out mr-2;
}
...
<template>
  <main class="flex-grow container mx-auto px-4 sm:px-6 pb-2 md:pb-4">
    <h1 class="text-3xl md:text-4xl font-bold text-teal-700 mb-4 text-center tracking-tight">
      은행 지도 및 경로 검색
    </h1>
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      <!-- Controls Panel -->
      <div class="lg:col-span-4 bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-lg">
        <!-- Start Location Settings -->
        <section class="mb-8">
          <h2 class="text-xl font-semibold text-gray-800 mb-5 border-b pb-3">출발지 설정</h2>
          <div class="space-y-4">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">출발지 유형</label>
              <div class="flex items-center space-x-6 bg-white p-3 rounded-lg border border-gray-300">
            ...
            <div v-if="startLocationType === 'custom'" class="space-y-3">
              <label class="block text-sm font-medium text-gray-700">출발 주소</label>
        ...
        <section>
          <h2 class="text-xl font-semibold text-gray-800 mb-5 border-b pb-3">도착 은행 검색</h2>
          <div class="space-y-5">
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">광역시 / 도</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">시 / 군 / 구</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1.5">은행</label>
      ...
      <!-- Map Area -->
      <div class="lg:col-span-8 bg-gray-50 p-4 sm:p-6 rounded-xl border border-gray-200 shadow-lg">
        <h3 class="text-xl font-semibold text-gray-800 mb-4 text-center">지도 및 경로 정보</h3>
        <div ref="map1El" class="map-container-custom"></div>

        <div
          class="mt-6 flex flex-col sm:flex-row justify-center items-center space-y-3 sm:space-y-0 sm:space-x-6 bg-white p-3 rounded-lg border border-gray-300 shadow-sm">
          <label class="radio-label-custom">
        ...
        <div ref="routeInfoEl" id="routeInfo"
          class="mt-5 text-center text-teal-700 font-semibold text-lg bg-teal-50 p-3 rounded-md">
