...
.form-input-custom {
  @apply w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm placeholder-gray-400 
  focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition duration-150 ease-in-out;
  font-size: 0.95rem;
}

.form-select-custom {
  @apply w-full pl-4 pr-10 py-2.5 border border-gray-300 rounded-lg shadow-sm bg-white
  focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition duration-150 ease-in-out;
  font-size: 0.95rem;
  appearance: none;
...
.btn-primary {
  @apply w-full bg-teal-600 text-white font-semibold py-3 px-4 rounded-lg shadow-md
  hover:bg-teal-700 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2
  transition duration-150 ease-in-out;
}

.btn-secondary {
  @apply w-full bg-sky-500 text-white font-semibold py-2.5 px-4 rounded-lg shadow-md
...
.radio-custom {
  @apply appearance-none h-5 w-5 border-2 border-gray-400 rounded-full 
  checked:bg-teal-500 checked:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-400 focus:ring-offset-1
  transition duration-150 ease-in-out mr-2;
}

.map-container-custom {
  min-height: 550px; /* Slightly adjusted */
  width: 100%;
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
      ...
      <div class="lg:col-span-4 bg-gray-50 p-6 rounded-xl border border-gray-200 shadow-lg">
              ...
              <div>
                ...
                <div class="flex items-center space-x-6 bg-white p-3 rounded-lg border border-gray-300">
                  <label class="radio-label-custom">
                    <input type="radio" class="radio-custom" name="startLocationType" value="current" v-model="startLocationType">
                    현재 위치
                  </label>
                  <label class="radio-label-custom">
                    <input type="radio" class="radio-custom" name="startLocationType" value="custom" v-model="startLocationType">
                    직접 입력
                  </label>
                </div>
              </div>
        ...
        <div class="lg:col-span-8 bg-gray-50 p-4 sm:p-6 rounded-xl border border-gray-200 shadow-lg">
          ...
          <div class="mt-6 flex flex-col sm:flex-row justify-center items-center space-y-3 sm:space-y-0 sm:space-x-6 bg-white p-3 rounded-lg border border-gray-300 shadow-sm">
            <label class="radio-label-custom">
              <input type="radio" class="radio-custom" name="transport" value="car" v-model="selectedTransportType" @change="handleTransportChange">
              자동차 경로
            </label>
            <!-- Add other transport types if needed in the future -->
          </div>

          <div ref="routeInfoEl" id="routeInfo" class="mt-5 text-center text-teal-700 font-semibold text-lg bg-teal-50 p-3 rounded-md">
