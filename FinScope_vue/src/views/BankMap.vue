    ...
    <div class="bg-white p-6 md:p-8 rounded-xl shadow-xl border border-gray-200">
      ...
      <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="md:col-span-3 bg-gray-50 p-4 rounded-lg border border-gray-200">
          ...
          <div ref="routeInfoEl" id="routeInfo" class="mt-4 text-center text-gray-700 font-medium"></div>
        </div>
      </div>
    </div>
  </main>
...
const routeInfoEl = ref(null); // To display route information, connected to the div
...
function tryInitializeMap() {
  if (window.kakao && window.kakao.maps) {
    // Check for services specifically if needed by initializeMaps or other functions
    if (window.kakao.maps.services) {
      console.log('Kakao Maps SDK (with services) ready, initializing map.');
      initializeMaps();
      loadDropdownData();
    } else {
      // This case might indicate the 'services' library failed to load with the main SDK
      console.error('Kakao Maps SDK loaded, but "services" library is missing. Check script URL in index.html.');
      alert('지도 서비스 라이브러리를 불러오는데 실패했습니다. 관리자에게 문의하세요.');
    }
  } else {
    console.log('Kakao Maps SDK core not ready yet, retrying in 100ms...');
    setTimeout(tryInitializeMap, 100); // Poll every 100ms
  }
}

onMounted(() => {
  console.log('BankMap.vue mounted, attempting to initialize map...');
  tryInitializeMap();
});
</script>
