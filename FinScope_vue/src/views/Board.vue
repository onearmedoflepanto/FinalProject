<template>
  <main class="flex-grow container mx-auto px-4 sm:px-6 py-6 md:py-8"> {/* Adjusted top padding */}
    <div id="postListSection" class="board-section p-4 md:p-6"
          ...
          <tbody id="postList" class="bg-white divide-y divide-gray-200">
            ...
            <tr v-for="post in posts" :key="post.id" class="hover:bg-gray-50 transition-colors duration-150">
              ...
              <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-900 w-6/12 truncate">
                <a href="#" class="text-teal-600 hover:text-teal-800 hover:underline font-medium" @click.prevent="showPostDetail(post.id)">
                  {{ post.title }} 
                  <span v-if="post.comments_count > 0" class="text-xs text-gray-500 ml-1">[{{ post.comments_count }}]</span>
                </a>
              </td>
              <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-600 w-2/12">{{ post.author_nickname || post.author }}</td> {/* Assuming author_nickname might be available */}
              <td class="py-4 px-4 whitespace-nowrap text-sm text-gray-600 w-2/12">{{ formatDate(post.created_at) }}</td>
    ...
    <PostDetail v-if="currentSection === 'postDetailSection'" :post="currentPost" :comments="comments"
      :on-back-to-list="showPostList" @toggle-like="toggleLike" @toggle-bookmark="toggleBookmark" @edit-post="editPost"
...
// import Footer from '../components/footer.vue'; // Footer import removed
