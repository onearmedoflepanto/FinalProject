  ...
  <div class="content-section p-6 sm:p-8">
    ...
    <form @submit.prevent="handleProfileEditSubmit" class="space-y-6">
      <div class="grid grid-cols-1 gap-y-4"> 
        ...
        <div><label for="editUserInvestmentTendency" class="form-label-dark-local">투자 성향</label>
          <select id="editUserInvestmentTendency" v-model="editData.investmentTendency" class="form-select-dark-local">
...
const editData = reactive({
  ...
  assets: null,
  salary: null,
  investmentTendency: '', // Changed from savingsTendency
  currentPassword: '',
  newPassword: '',
  confirmNewPassword: '',
});
...
const populateEditForm = (currentUser) => {
  if (currentUser) {
    ...
    editData.investmentTendency = currentUser.investment_tendency || currentUser.investmentTendency || ''; // Changed
  }
...
const handleProfileEditSubmit = async () => {
  ...
  const profileUpdateData = {
    ...
    investment_tendency: editData.investmentTendency, // Changed
