<template>
  <div class="content-section p-6 sm:p-8">
    ...
    <div v-if="userData" class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
      ...
      <div><label class="block text-xs font-semibold text-gray-400 uppercase tracking-wider mb-1">저축 성향</label><p class="text-gray-100 text-base">{{ userData.savingsTendency || '정보 없음' }}</p></div>
    </div>
