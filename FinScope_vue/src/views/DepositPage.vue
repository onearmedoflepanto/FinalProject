<template>
  <div class="bg-gray-900 text-gray-100 min-h-screen py-8 md:py-12">
      ...
      <div id="productListSection" class="page-section bg-gray-800 p-6 md:p-8 rounded-xl shadow-xl border border-gray-700">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
          ...
          <div class="w-full sm:w-auto">
            ...
            <select id="bankFilter" class="form-select w-full sm:w-72 rounded-md" v-model="selectedBank"
              ...
              <transition name="slide-fade">
                <tr v-if="expandedProductId === product.id" class="product-detail-row bg-gray-700/30">
                  <td colspan="6" class="p-0 border-b border-gray-600">
                    <div class="p-4 bg-gray-700 rounded-b-lg detail-content-wrapper">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 mb-4 detail-section">
                    ...
                    <h3 class="text-lg font-semibold text-gray-100 mb-2 mt-6 pt-4 border-t border-gray-600">금리 정보 (세전)</h3>
                    <div class="space-y-1 detail-section py-2">
...
const fetchData = async () => {
  try {
    ...
    if (data && data.result && data.result.baseList && data.result.optionList) {
      const baseProducts = data.result.baseList;
      const optionList = data.result.optionList;

      const processedProducts = baseProducts.map(baseProduct => {
        const productOptions = optionList.filter(
          opt => opt.fin_co_no === baseProduct.fin_co_no && opt.fin_prdt_cd === baseProduct.fin_prdt_cd
        );

        const rates = {};
        productOptions.forEach(opt => {
          if (opt.save_trm) {
            // Use intr_rate2 if available and higher, otherwise intr_rate
            // This is an assumption, adjust if intr_rate should always be primary
            rates[opt.save_trm] = (opt.intr_rate2 !== null && opt.intr_rate2 > (opt.intr_rate || 0)) ? opt.intr_rate2 : opt.intr_rate;
          }
        });

        return {
          ...baseProduct, // Spread all properties from baseList item
          id: `${baseProduct.fin_co_no}_${baseProduct.fin_prdt_cd}`, // Composite unique ID
          bank: baseProduct.kor_co_nm,
          name: baseProduct.fin_prdt_nm,
          rates: rates,
          submitMonth: baseProduct.dcls_month,
          restriction: baseProduct.join_deny === '1' ? '제한없음' : (baseProduct.join_deny === '2' ? '서민전용' : (baseProduct.join_deny === '3' ? '일부제한' : '기타')), // Example mapping for join_deny
          way: baseProduct.join_way,
          special: baseProduct.spcl_cnd,
          // Ensure all fields used in the template are mapped here
          // fin_co_no and fin_prdt_cd are already in baseProduct
          product_type: currentProductType.value // Add product_type for saving
        };
      });
...
.form-select {
  @apply bg-gray-700 border-gray-600 text-gray-100 focus:ring-primary-light focus:border-primary-light py-2.5 px-3;
...
.product-name-link {
  @apply text-sky-400 hover:text-sky-300 font-medium; /* Ensure visibility */
  cursor: pointer;
}
...
/* Ensure the content inside the td doesn't get squashed during transition */
.product-detail-row td {
  overflow: hidden;
}
.detail-content-wrapper {
  /* You can set a max-height if needed for smoother animation,
     but it might clip content if not calculated correctly.
     For now, relying on opacity and transform. */
}
</style>
