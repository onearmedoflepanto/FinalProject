...
async function loadData() {
  ...
  const rawData = await fetchCommodityData(currentCommodity.value)

  chartData.value = filterData(rawData.map(({ Date: dateStr, Price }) => ({
...
function renderChart() {
  ...
  const formattedLabels = chartData.value.map(item =>
